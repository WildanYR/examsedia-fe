  ...
  <div
    ...
    <div class="w-full flex-1">
      <div class="mb-1 flex flex-col items-center justify-between md:flex-row">
        <span class="text-base font-medium dark:text-white">Progres Mengerjakan</span>
        <span class="text-sm font-medium dark:text-white">{{ props.soalDikerjakan }} / {{ props.jumlahSoal }}</span>
      </div>
      <div class="h-2.5 w-full rounded-full bg-gray-200 dark:bg-gray-700">
        <div class="h-2.5 rounded-full bg-blue-600" :style="'width:' + soalPercentage"></div>
      </div>
    </div>
    <div class="flex flex-none items-center gap-4">
      ...
      <div v-show="props.timer" class="group relative rounded-full bg-red-100 p-2 text-red-600">
        ...
        <div
          ...
          ujian ini memiliki batas waktu
        </div>
      </div>
    </div>
  </div>
