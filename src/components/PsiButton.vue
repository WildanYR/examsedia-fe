...
const variantClass = computed(() => {
  ...
  switch (props.variant) {
    case 'primary':
      className =
        'bg-primary text-white hover:bg-blue-800 active:bg-blue-800 focus:outline-none focus:ring focus:ring-blue-300 transition-colors disabled:bg-blue-200 disabled:text-blue-600'
      break
    case 'red':
      className =
    ...
    case 'red-light':
      className =
        'bg-red-50 text-red-700 hover:bg-red-100 focus:outline-none focus:ring-0 transition-colors disabled:bg-gray-200 disabled:text-gray-600'
      break
    default:
      className = ''
      break
...
<template>
  <button :disabled="props.loading" :class="['rounded-md tracking-wide', variantClass, sizeClass]">
    <loading-spinner v-if="props.loading" :class="['mr-2 inline h-4 w-4 animate-spin', spinnerClass]" />
    <slot></slot>
  </button>
</template>
