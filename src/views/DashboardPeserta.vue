...
<template>
  ...
  <template v-else-if="psikotesActive">
    <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
      ...
      <div class="flex flex-col items-center rounded-xl bg-white p-4 shadow-md md:col-span-2"></div>
      <widget-jam />
    </div>
    <widget-grid-container title="Daftar Alat Tes" description="dibawah ini adalah daftar alat tes yang bisa dikerjakan pada sesi saat
          ini">
      <alat-tes-box v-for="alatTes in alatTesData" :key="alatTes.id" :variant="alatTes.selesai ? 'green' : 'blue'"
        :title="alatTes.nama">
        <p v-if="alatTes.selesai" class="text-green-900">Telah dikerjakan</p>
        <psi-button v-else variant="black" :loading="loadingKerjakan" class="w-full"
          @click="handleKerjakan(alatTes.id)">Kerjakan</psi-button>
      </alat-tes-box>
    </widget-grid-container>
  </template>
  <div v-else class="flex items-center justify-center">
    <h2 class="text-3xl font-medium text-red-300">
      Sesi Ujian tidak dilaksanakan
    </h2>
  </div>
</template>
