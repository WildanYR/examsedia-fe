...
import { TrashIcon, PhotographIcon, DocumentTextIcon } from '@heroicons/vue/outline'
...
const addNewSoal = () => {
  const soalNo = soalData.value.length + 1
  soalData.value.push({
    nomor: soalNo.toString(),
    teks: '',
...
<template>
  <widget-container title="Edit Soal" :loading="loadingGet">
    ...
    <psi-dropdown v-model="defaultJenisSoal" label="Jenis Soal Dasar" :items="jenisSoal" />
  </widget-container>
  ...
  <template v-else>
    ...
    <div v-else>
      <div v-if="soalData.length" class="flex flex-col gap-10">
        <div v-for="(soal, i) in soalData" :id="`soal-${i}`" :key="`soal-${i}`"
          class="rounded-lg bg-white p-6 border border-slate-300">
          <div class="mx-auto max-w-4xl space-y-5">
            ...
            " v-model="soal.jenis_soal" label="Jenis Soal" :items="defaultJenisSoal === 'pilgan_x' ||
              defaultJenisSoal === 'pilgan_y'
              ? jenisSoalPgOpsi
              : jenisSoalMcOpsi
              " />
            <psi-editor v-model="soal.teks" label="Narasi Soal" />
            <form-input v-if="soal.jenis_soal === 'kelompok'" v-model="soal.opsi_soal" label="Kelompok Jawaban" />
            <div v-else-if="
              [
                'pilgan_y',
                'pilgan_x',
                ...
                <div v-for="(opsi, j) in soal.opsi_soal" :key="`${soal.id}-opsi-${j}`">
                  <div class="flex items-center gap-2 mb-2">
                    ...
                    <psi-button v-if="opsi.type === 'text'" @click="changeOpsiType(i, j, 'image')"><photograph-icon
                        class="h-5 w-5" /></psi-button>
                    <psi-button v-else @click="changeOpsiType(i, j, 'text')"><document-text-icon
                        class="h-5 w-5" /></psi-button>
                  </div>
                  <img loading="lazy" :src="generateImageUrl(opsi.teks)" class="w-24"
                    :class="opsi.type !== 'image' && 'hidden'" />
                  <form-input v-model="opsi.teks" label="Narasi" class="flex-1"
                    :class="opsi.type !== 'text' && 'hidden'" />
                </div>
              </div>
              <psi-button class="mt-7 w-full" @click="addNewOpsi(i)">Tambah Opsi</psi-button>
            </div>
            <div v-else class="text-center text-gray-500">
              Jenis soal tidak membutuhkan opsi
            </div>
            <psi-button variant="red-outline" class="w-full" @click="removeSoal(i)">Hapus Soal</psi-button>
          </div>
        </div>
      </div>
      <psi-button variant="black" class="mt-6 w-full" @click="addNewSoal">Tambah Soal</psi-button>
      <div class="mt-6 flex items-center gap-4">
        <psi-button class="flex-1" variant="red" @click="$router.go(-1)">Kembali</psi-button>
        <psi-button :loading="loadingMutate" class="flex-1" @click="handleMutateSoal">Simpan Soal</psi-button>
      </div>
    </div>
  </template>
  <modal-image :show-modal="showImageModal" @close-modal="closeImageModal"
    @image-selected="handleImageSelected"></modal-image>
</template>
