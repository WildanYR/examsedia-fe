...
<template>
  ...
  <div class="space-y-4">
    <form-input v-model="formData.email" label="Email"
      :error-message="formDataValidate.email.errorMessage"></form-input>
    <div>
      <form-input v-model="formData.password" type="password" label="Kata Sandi"
        :error-message="formDataValidate.password.errorMessage" @keyup.enter="handleLogin"></form-input>
      <router-link to="/" class="ml-1 text-sm text-blue-800 hover:text-blue-700">
        Lupa kata sandi?
      </router-link>
    </div>
    <psi-button class="w-full" :loading="loadingLogin" @click="handleLogin">Login</psi-button>
    <div v-if="authStore.registerStatus" class="text-center">
      <router-link :to="{ name: 'register' }" class="text-blue-800 hover:text-blue-700">
        Belum punya akun?
      </router-link>
    </div>
  </div>
