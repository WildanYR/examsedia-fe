...
<template>
  <widget-grid-container title="Daftar Alat Tes" description="dibawah ini adalah daftar semua alat tes"
    :loading="loadingGetAlatTes">
    <template #cta>
      <loading-button @click="openAddModal">Tambah Alat Tes</loading-button>
    </template>
    <alat-tes-box v-for="alatTes in dataAlatTes" :key="alatTes.id" :title="alatTes.nama"
      :subtitle="`${alatTes.kelompok_tes_count} Kelompok Tes`" variant="blue">
      <div class="flex w-full items-center gap-3">
        <psi-button class="w-full" variant="amber" @click="redirectEdit(alatTes.id)">Edit</psi-button>
        <psi-button class="w-full" variant="red" @click="openDeleteModal(alatTes)">Hapus</psi-button>
      </div>
    </alat-tes-box>
  </widget-grid-container>

  <psi-modal size="2xl" title="Tambah Alat Tes" :show-modal="showAddModal" @close-modal="closeAddModal">
    <div class="space-y-4">
      <form-input v-model="formAdd.nama" label="Nama" :error-message="formAddValidation.nama.errorMessage" />
    ...
    <div class="mt-3 flex items-center justify-end gap-3">
      ...
      <psi-button :loading="loadingAddAlatTes" @click="handleAdd">Tambah</psi-button>
    </div>
  </psi-modal>
  <psi-modal size="xl" title="Hapus Alat Tes" :show-modal="showDeleteModal" @close-modal="closeDeleteModal">
    ...
    <div class="mt-3 flex items-center justify-end gap-3">
      ...
      <psi-button variant="red" :loading="loadingDeleteAlatTes" @click="handleDelete">Hapus</psi-button>
    </div>
  </psi-modal>
</template>
