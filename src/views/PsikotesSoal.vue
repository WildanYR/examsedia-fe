...
<template>
  <div class="relative">
    <psikotes-navbar :alat-tes="alatTesData.nama" :kelompok-tes="kelompokTesData.nama"
      :timer="!!psikotesStore.timer_start" :soal-dikerjakan="progresMengerjakan.soalDikerjakan"
      :jumlah-soal="progresMengerjakan.jumlahSoal" class="mb-5"></psikotes-navbar>
    <div class="flex flex-col gap-5">
      <div v-for="soal in soalData" :key="soal.id" class="rounded-lg bg-white px-10 py-8 border border-gray-300">
        <div class="mx-auto max-w-4xl">
          ...
          <div class="mx-auto mt-7 max-w-4xl">
            <psikotes-pilgan v-if="
              soal.jenis_soal === 'pilgan_y' || soal.jenis_soal === 'pilgan_x'
            " :model-value="getJawaban(soal.id)" :opsi="soal.opsi_soal"
              :direction="soal.jenis_soal === 'pilgan_y' ? 'y' : 'x'"
              @update:model-value="(val) => handleJawab(soal.id, val)"></psikotes-pilgan>
            <form-input v-else-if="soal.jenis_soal === 'uraian'" :model-value="getJawaban(soal.id)" label="Jawaban"
              @update:model-value="(val) => handleJawab(soal.id, val)" />
            <psikotes-angka v-else-if="soal.jenis_soal === 'angka'" :model-value="getJawaban(soal.id)"
              @update:model-value="(val) => handleJawab(soal.id, val)" />
            <psikotes-multichoice v-else-if="
              soal.jenis_soal === 'multichoice_y' ||
              soal.jenis_soal === 'multichoice_x'
            " :model-value="getJawaban(soal.id)" :opsi="soal.opsi_soal"
              :direction="soal.jenis_soal === 'multichoice_y' ? 'y' : 'x'"
              @update:model-value="(val) => handleJawab(soal.id, val)" />
            <psikotes-opini v-else-if="soal.jenis_soal === 'opini'" :model-value="getJawaban(soal.id)"
              @update:model-value="(val) => handleJawab(soal.id, val)" />
            <psikotes-kelompok v-else-if="soal.jenis_soal === 'kelompok'" :model-value="getJawaban(soal.id)"
              :opsi="soal.opsi_soal" @update:model-value="(val) => handleJawab(soal.id, val)" />
            <div v-else-if="soal.jenis_soal === 'membaca'" class="text-center text-gray-500">
              tidak perlu jawaban
            </div>
